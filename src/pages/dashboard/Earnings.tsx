    ...
    <div className="flex flex-col min-h-screen bg-background pb-24 relative overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-secondary to-[#FFE082] dark:from-secondary dark:to-primary/20 rounded-b-3xl border-b border-primary/20 pt-12 pb-8 px-5 max-w-md mx-auto w-full relative z-10 shadow-sm mb-6">
        ...
        <div className="flex bg-background/60 backdrop-blur-md rounded-2xl p-1 mb-6 border border-primary/10">
          <button className="flex-1 py-2 bg-background rounded-xl shadow-sm text-[13px] font-bold text-foreground">{t("today") || "Today"}</button>
          <button className="flex-1 py-2 text-[13px] font-semibold text-foreground/60 hover:text-foreground transition-colors">{t("week") || "Week"}</button>
          <button className="flex-1 py-2 text-[13px] font-semibold text-foreground/60 hover:text-foreground transition-colors">{t("month") || "Month"}</button>
        </div>
      ...
      <div className="px-5 space-y-4 max-w-md mx-auto w-full relative z-10 -mt-2">
        ...
        <div className="bg-card rounded-2xl border border-border shadow-sm p-5">
          <p className="text-[16px] font-extrabold text-foreground mb-6">{t("weeklyPerformance") || "Weekly Performance"}</p>
        ...
        <div className="pt-2 flex justify-between items-center">
          <h2 className="text-[16px] font-extrabold text-foreground">{t("recentPayouts") || "Recent Payouts"}</h2>
          <button className="text-[12px] font-bold text-accent uppercase tracking-wider">{t("seeAll") || "See All"}</button>
        </div>
          ...
          {[1, 2, 3].map((_, i) => (
            <div key={i} className="bg-card rounded-2xl border border-border shadow-sm p-4 flex items-center justify-between hover:bg-muted active:scale-[0.98] transition-transform cursor-pointer">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-[14px] font-bold text-foreground">{t("weeklyPayout") || "Weekly Payout"}</p>
